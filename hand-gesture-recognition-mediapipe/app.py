...
def main():
    ...
    while True:
        ...
        # Process Key (ESC: end, Enter: append gesture) #####
        key = cv.waitKey(10)
        if key == 27:  # ESC
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                current_gesture = keypoint_classifier_labels[hand_sign_id]  # Update the current gesture
        ...
        debug_image = draw_info(debug_image, fps, mode,number)  # Removed 'number'
